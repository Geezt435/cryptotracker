...
<body>
    <div class="container">
        <h1 class="my-4">Pantau Koin RHN</h1>
            ...
            fetch(requestUrl, {
                ...
                .then(data => {
                    const coinTable = $('#coinTable').DataTable({
                        ...
                        columns: [
                            ...
                            },
                            {
                                data: null,
                                render: function (data, type, row) {
                                    return '<input type="checkbox" name="symbol[]" value="' + row.symbol + '">';
                                }
                            },
                            {
                                data: null,
                    ...
                    $('#filterBtn').on('click', function () {
                        const watchlistInput = $('#watchlistInput').val().split(',').map(item => item.trim());
                        coinTable.columns([2]).search(watchlistInput.join('|'), true, false).draw();
                    });
                })
                .catch(error => {
